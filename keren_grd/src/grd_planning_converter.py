...
class grd_converter:
    ...
    def grd_to_split(self, domain_file_name, template_file_name, hyps_file_name, destination_folder_name,
                     epsilon=DEFAULT_EPSILON, log_stream=None, is_poi_grd=False):
        '''
        ...
        [split_domain_file_name, predicates_to_duplicate] = self.generate_domain_file(os.path.abspath(domain_file_name),
                                                                                      os.path.abspath(
                                                                                          destination_folder_name),
                                                                                      epsilon, is_poi_grd)

        # generate split problem file
        split_problem_file_name = self.generate_split_problem_file(template_file_name, hyps_file_name,
                                                                   predicates_to_duplicate, destination_folder_name,
                                                                   is_poi_grd)
    ...
    def create_actions_list(self, planning_domain, epsilon, create_together_actions=True, create_split_action=True,
                            create_done_action=True):
        ...
        if create_split_action:
            ...
            effect = pddl.Effect()
            effect.addlist = [self.action_predicates[split_string],
                              pddl.Predicate(action_cost_predicate_string % 0, [])]
    ...
    def generate_split_problem_file(self, template_file_name, hyp_file_name, predicates_to_duplicate,
                                    destination_folder_name, is_poi_grd=False):
        '''
            ...
            while line and not goal_line_reached:
                ...
                if '(:objects' in line.lower():
                    ...
                    if '(:init' in line.lower():
                        ...
                        line_to_print = self.generate_init_state_stmt(line, instream_template, len(hyps),
                                                                      predicates_to_duplicate, True)
    ...
    def get_action_cost(self, action=None, planning_domain=None, is_discounted=False, is_together=False,
                        epsilon=DEFAULT_EPSILON):
        if is_discounted:
            discount = epsilon
        else:
    ...
    def generate_domain_file(self, domain_file_name, destination_folder_name, epsilon, is_poi_grd=False):
        ...
        grd_planning_gen.generate_domain_file(planning_domain, split_domain_file_name, requirements_string,
    ...
    def generate_together_action(self, action, planning_domain):
        ...
        for predicate in action.precondition:
            ...
            if predicate.name == '=':
                ...
                predicate_0 = pddl.Predicate(copy.deepcopy(predicate.name), copy.deepcopy(predicate.signature),
                                             copy.deepcopy(predicate.IsNot))
                predicate_1 = pddl.Predicate(copy.deepcopy(predicate.name), copy.deepcopy(predicate.signature),
                                             copy.deepcopy(predicate.IsNot))
    ...
    def generate_separate_action(self, action, agent_num, planning_domain, indicate_split=True, is_after_split=True,
                                 is_discounted=False, add_done_conditions=True, assign_original_cost=True,
                                 add_paid_indication=False):
        ...
        for predicate in action.precondition:
            new_predicate = pddl.Predicate(copy.deepcopy(predicate.name), copy.deepcopy(predicate.signature),
                                           copy.deepcopy(predicate.IsNot))

            if (
                    predicate.name != '=' and predicate.name not in self.static_predicates) or grd_defs.duplicate_all_predicates:
    ...
    def generate_init_state_stmt(self, init_line, instream_template, num_of_hyps, predicates_to_duplicate,
                                 gen_closing_bracet=True):
        '''
...
class grd_converter_timedLatestSplit(grd_converter):
    ...
    def extend_domain_predicates(self, planning_domain):
        ...
        if (planning_domain.actionCosts == False):
            ...
            self.action_predicates[timer_action_predicate_string % '1'] = pddl.Predicate(
                timer_action_predicate_string % '1', [])
        ...
        # non uniform action costs
            ...
            for cost in self.actionsCostsList:
                planning_domain.predicates[timer_predicate_string % cost] = pddl.Predicate(
                    timer_predicate_string % cost, [])
                self.action_predicates[timer_action_predicate_string % cost] = pddl.Predicate(
                    timer_action_predicate_string % cost, [])
    ...
    # generate a copy of an action performed seperately by agent agent_num
    def generate_separate_action(self, action, agent_num, planning_domain, is_after_split=True, is_discounted=False,
                                 add_done_conditions=True):

        action_separate = grd_converter.generate_separate_action(self, action, agent_num, planning_domain, True, True,
                                                                 False, add_done_conditions)
    ...
    def generate_init_state_stmt(self, init_line, instream_template, num_of_hyps, predicates_to_duplicate,
                                 gen_closing_bracket=True):
        init_stmt = grd_converter.generate_init_state_stmt(self, init_line, instream_template, num_of_hyps,
                                                           predicates_to_duplicate, False)
...
class grd_converter_latestExpose(grd_converter_timedLatestSplit):
    ...
    def generate_separate_action(self, action, agent_num, planning_domain):
        ...
        if not self.budget_pair:
            ...
            action_separate_observable = grd_converter.generate_separate_action(self, action, agent_num,
                                                                                planning_domain, True, True, False)
            # we discount only non observable actions performed by agent 0
            if agent_num == 0:
                action_separate_non_observable = grd_converter.generate_separate_action(self, action, agent_num,
                                                                                        planning_domain, True, False,
                                                                                        True)
            else:
                action_separate_non_observable = grd_converter.generate_separate_action(self, action, agent_num,
                                                                                        planning_domain, True, False,
        ...
        # suboptimal
            ...
            # the flags indicate is_split(for action peformed after split and ) and is_discounted (for discounted actions)
            action_separate_observable = grd_converter_timedLatestSplit.generate_separate_action(self, action,
                                                                                                 agent_num,
                                                                                                 planning_domain, True,
                                                                                                 True, False)
            # we discount only non observable actions performed by agent 0
            if agent_num == 0:
                action_separate_non_observable = grd_converter_timedLatestSplit.generate_separate_action(self, action,
                                                                                                         agent_num,
                                                                                                         planning_domain,
                                                                                                         True, False,
                                                                                                         True)
        ...
        if self.are_observables_specified:
            ...
            # observability
            action_separate_non_observable.precondition.append(
                self.action_predicates[not_observable_predicate_string % action.name])
        ...
        # if the non-observable actions are specified (we need the action precondition to be (non-observable_unstack ?x - block ?y - block))
            ...
            # observability
            action_separate_non_observable.precondition.append(
                self.action_predicates[non_observability_predicate_string % action.name])
            action_separate_non_observable.name += '_' + non_observable_string
    ...
    def generate_init_state_stmt(self, init_line, instream_template, num_of_hyps, predicates_to_duplicate,
        ...
        if not self.budget_pair:
            ...
            init_stmt = grd_converter_timedLatestSplit.generate_init_state_stmt(self, init_line, instream_template,
                                                                                num_of_hyps, predicates_to_duplicate,
                                                                                False)
...
class grd_converter_commmonDeclare(grd_converter_timedLatestSplit):
    ...
    def generate_init_state_stmt(self, init_line, instream_template, num_of_hyps, predicates_to_duplicate,
                                 generate_closing_bracket=False):
        # optimal agents (False indicates not generating the closing bracket)
        if not self.budget_pair:
            init_stmt = grd_converter.generate_init_state_stmt(self, init_line, instream_template, num_of_hyps,
                                                               predicates_to_duplicate, False)
    ...
    def add_declare_actions(self, new_action_list):
        ...
        for agent_index in [0, 1]:
            ...
            # no pending token
            effect.addlist.append(
                pddl.Predicate('%s agent_%s' % (pending_token_declared_predicate_string, agent_index), []))
            effect.addlist.append(pddl.Predicate('exposed', []))
    ...
    def add_pay_actions(self, new_action_list, epsilon=DEFAULT_EPSILON):
        ...
        for cost in cost_list:
            ...
            effect = pddl.Effect()
            # cost
            effect.addlist = [pddl.Predicate('paid_agent_0', []),
                              pddl.Predicate(action_cost_predicate_string % (factored_cost), [])]
            ...
            effect = pddl.Effect()
            effect.addlist = [pddl.Predicate('paid_agent_0', []),
                              pddl.Predicate(action_cost_predicate_string % (factored_cost * (1 - 0.009)), []),
                              pddl.Predicate('not_discounted_agent_0', [])]
            # cost
    ...
    def create_actions_list(self, planning_domain, epsilon):
        ...
        # non optimal agents
            ...
            new_action_list = grd_converter_timedLatestSplit.create_actions_list(self, planning_domain, epsilon, False,
                                                                                 True)
    ...
    def generate_separate_action_old(self, action, agent_num, planning_domain, isZeroParams=False):
        ...
        if not self.budget_pair:
            # generate separate
            action_separate = grd_converter.generate_separate_action(self, action, agent_num, planning_domain, False,
                                                                     False, False, False)
            action_separate.signature.append(['?tok', ('act_tok')])

            # generate a sperate action that emits not token
            action_seperate_nil_token = grd_converter.generate_separate_action(self, action, agent_num, planning_domain,
                                                                               False, False, False, False)
            action_seperate_nil_token.name += '_%s' % grd_defs.NIL_STRING

            # generate a sperate action that emits not token
            action_seperate_nil_token_params = grd_converter.generate_separate_action(self, action, agent_num,
                                                                                      planning_domain, False, False,
                                                                                      False, False)
        ...
        # suboptimal
            ...
            # generate separate
            action_separate = super(grd_converter_timedLatestSplit, self).generate_separate_action(action, agent_num,
                                                                                                   planning_domain,
                                                                                                   False, False, False,
                                                                                                   False)
            action_separate.signature.append(['?tok', ('act_tok')])
            ...
            action_seperate_nil_token = super(grd_converter_timedLatestSplit, self).generate_separate_action(action,
                                                                                                             agent_num,
                                                                                                             planning_domain,
                                                                                                             False,
                                                                                                             False,
            ...
            action_seperate_nil_token_params = super(grd_converter_timedLatestSplit, self).generate_separate_action(
                action, agent_num, planning_domain, False, False, False, False)
        ...
        # no pending token
        action_separate.precondition.append(
            pddl.Predicate('%s agent_%s' % (pending_token_declared_predicate_string, agent_num), []))
        # emitted token
        emitted_token_predicate = pddl.Predicate(token_emitted_string, [])
        ...
        action_separate.effect.dellist.append(
            pddl.Predicate('%s agent_%s' % (pending_token_declared_predicate_string, agent_num), []))
        ...
        # no pending token
        action_seperate_nil_token.precondition.append(
            pddl.Predicate('%s agent_%s' % (pending_token_declared_predicate_string, agent_num), []))

        # nil token action ( the nil token does not require any declaration)
        action_token_string_nil_params = 'action_token_ParamsNum_%s %s %s %s' % (
        param_count, action_name, 'AT_NIL', params_string)
        ...
        # no pending token
        action_seperate_nil_token_params.precondition.append(
            pddl.Predicate('%s agent_%s' % (pending_token_declared_predicate_string, agent_num), []))
    ...
    def generate_separate_action(self, action, agent_num, planning_domain, isZeroParams=False):
        ...
        if not self.budget_pair:
            ...
            if agent_num == 0:  # don;t assign regular cost (zero cost)
                action_separate = grd_converter.generate_separate_action(self, action, agent_num, planning_domain,
...
def convert_grd_to_planning():
    ...
    if grd_defs.LatestSplit == calc_method:
        ...
        grd_test_task.initialize_with_files(grd_defs.gen_files_dir, domain_file_name, template_file_name,
                                            hyps_file_name)
        ...
        grd_test_task = grd_task.GrdTask()
        grd_test_task.initialize_with_files(destination_folder_name, domain_file_name, template_file_name,
                                            hyps_file_name, observability_file_name)
        ...
        grdConverter = grd_converter_latestExpose(observability_file_name, grd_test_task.are_observables_specified,
                                                  optimal_costs, grd_test_task.get_sub_optimal_bound_array())
        grdConverter.grd_to_split(domain_file_name, template_file_name, hyps_file_name, destination_folder_name)
        return
        ...
        grdConverter = grd_converter_commmonDeclare(observability_file_name, optimal_costs,
                                                    grd_test_task.get_sub_optimal_bound_array())
        grdConverter.grd_to_split(domain_file_name, template_file_name, hyps_file_name, destination_folder_name)
        return
