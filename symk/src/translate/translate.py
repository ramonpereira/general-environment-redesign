...
def get_next_state(state, action):
    for del_effect in action.del_effects:
        del_eff = del_effect[1]
        try:
            if del_eff in state:
                state.remove(del_eff)
        except:
            e = sys.exc_info()[0]
            print(e)
            continue
    for add_effect in action.add_effects:
        add_eff = add_effect[1]
        state.append(add_eff)
    return state

def get_plans(plans_directory):
    plans = []
    plan_files = os.listdir(plans_directory)
    for plan_file in plan_files:
        infile = open(f'{plans_directory}{plan_file}')
        plan = []
        for line in infile:
...
def get_clean_state(s):
    clean_state = []
    for item in s:
        if isinstance(item, Atom):
            if item.predicate != '=':
                string = f'({item.predicate}'
                for arg in item.args:
                    string += f' {arg}'
                string += ')'
                clean_state.append(string)
        elif isinstance(item, NegatedAtom):
            string = f'(not ({item.predicate}'
            for arg in item.args:
                string += f' {arg}'
            string += '))'
            clean_state.append(string)
        elif isinstance(item, Assign):
            if isinstance(item.expression, NumericConstant):
                if len(item.fluent.args) == 0:
                    string = f'(= ({item.fluent.symbol}) {item.expression.value})'
                else:
                   string = f'(= ({item.fluent.symbol} {" ".join(item.fluent.args)}) {item.expression.value})'
            elif isinstance(item.expression, PrimitiveNumericExpression):
                string = f'(= ({item.expression.symbol} {item.expression.args[0]}) {item.fluent.value})' # TODO: with the 0 here Im assuming functions can only have one parameter
            clean_state.append(string)
        else:
            string = None
            print('ERROR: Item in statee is not recognized')
    clean_state = sorted(clean_state)
    clean_state = ' '.join(clean_state)
    return clean_state
...
def main():
    ...
    if options.get_states:
        plans_directory = 'found_plans/'
        plans = get_plans(plans_directory)
        relaxed_reachable, fluent_facts, actions, instantiated_goal, axioms, action_parameters = instantiate.explore(task)
        states = []
        states.append(get_clean_state(task.init))
        for plan in plans:
            curr_state = list(task.init)
            for action in plan:
                for grounded_action in actions:
                    if action == grounded_action.name:
                        curr_state = get_next_state(curr_state, grounded_action)
                        clean_state = get_clean_state(curr_state)
